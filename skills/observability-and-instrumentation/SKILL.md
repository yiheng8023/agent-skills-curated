...
description: Use when production behavior is not sufficiently visible, or when adding or reviewing logs, metrics, traces, alerts, service-level indicators, retries, queues, jobs, or external integrations.
metadata:
  source: https://github.com/addyosmani/agent-skills/tree/17214a29c429a19f7a9607f2c06f9d650ea87eb0/skills/observability-and-instrumentation
  license: MIT
...
> Telemetry must follow the system's privacy, retention, data-minimization,
> redaction, access-control, and cost policies. Never log secrets by default.

## Coordination and Authority

This Skill owns telemetry design and evidence quality. The canonical debugging
workflow owns root-cause investigation; performance optimization consumes
measurements for a known bottleneck; and shipping consumes health signals for
GO/NO-GO and rollback decisions. Do not install agents or SDKs, connect an
external telemetry service, expand data collection, change production
configuration, or deploy without authorization for that exact scope.
...
**NOT for:**
- Diagnosing a failure happening right now — use the canonical `diagnose` Skill (observability is what makes that workflow faster next time)
...
**Never log secrets, tokens, passwords, or full PII.** Apply the active privacy
policy, and involve the canonical security capability when telemetry crosses a
new trust boundary. Telemetry pipelines are a classic data-leak path: allowlist
fields and do not log whole request bodies.
...
Use the verification list in this Skill as the authoritative observability gate.
