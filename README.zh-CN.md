...
反馈，不是跨项目调研或标准的长期载体；有边界的调研与标准候选包后续应在单独授权下
交付到 `YIYUAN-CALIBRATION`。CALIBRATION 独立但联动，位于 MERIDIAN 矩阵之外，
原则上是 ASSETS 重塑阶段的临时参考和校准仓。`YIYUAN-ASSETS` 独立拥有准入权，并是
标准最终内置或知识库承载的目标仓。

当前 approved release 只是种子，不足以覆盖人机协作短板。无论未来是否创建单独规划的
生产级能力管理器，以证据驱动真实扩充 approved release 都是本仓核心目标。管理器设计稿
现可供复核，但新建仓库和实施仍受 MERIDIAN 拓扑影响闸门阻断。
...
- `docs/superpowers/specs/2026-07-15-production-capability-manager-design.md`：
  已获会话方向批准、待书面复核的生产级能力管理器设计；不授权建仓或实施；
